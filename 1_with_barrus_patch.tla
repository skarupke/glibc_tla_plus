(* Copyright (C) 2023 Malte Skarupke
...
quiesce_lowseq:
  ...
  if ((futexes[g_refs_index[g1]].state \div 2) > 0)
  {
  quiesce_wake_remaining:
...
cv_wait_loop:
  ...
  cv_wait_inner_loop:
    ...
    cv_wait_prepare_to_block:
      futexes[g_refs_index[g]].state := futexes[g_refs_index[g]].state + 2;
    cv_wait_reload_signals_again:
      signals := futexes[g_signals_index[g]].state;
      local_g1_start := g1_start;
      lowseq := IF (g1_start % 2) = g THEN signals ELSE ClearBit(local_g1_start, 0); 
  ...
  cv_wait_inner_loop_done_cmp_exchg:
...
\* BEGIN TRANSLATION (chksum(pcal) = "1caeaa99" /\ chksum(tla) = "11ec879a")
\* Label futex_wake_all of procedure futex_wake_all at line 85 col 5 changed to futex_wake_all_
\* Label futex_wait of procedure lock_mutex at line 126 col 9 changed to futex_wait_
\* Label check_for_waiters of procedure cv_signal at line 424 col 3 changed to check_for_waiters_
\* Procedure variable old_state of procedure lock_mutex at line 90 col 10 changed to old_state_
\* Procedure variable old_state of procedure unlock_mutex at line 132 col 10 changed to old_state_u
\* Procedure variable lowseq of procedure condvar_quiesce_and_switch_g1 at line 266 col 11 changed to lowseq_
\* Procedure variable local_wseq of procedure cv_wait at line 315 col 11 changed to local_wseq_
\* Procedure variable g of procedure cv_wait at line 316 col 11 changed to g_
\* Procedure variable local_wseq of procedure cv_signal at line 419 col 11 changed to local_wseq_c
\* Procedure variable g1 of procedure cv_signal at line 420 col 11 changed to g1_
\* Procedure variable do_futex_wake of procedure cv_signal at line 421 col 11 changed to do_futex_wake_
\* Procedure variable local_wseq of procedure cv_broadcast at line 473 col 11 changed to local_wseq_cv
\* Procedure variable g1 of procedure cv_broadcast at line 474 col 11 changed to g1_c
VARIABLES mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
          ...
          old_g1_start, r, s, lowseq_, local_wseq_, g_, seq, signals, 
          local_g1_start, lowseq, local_wseq_c, g1_, do_futex_wake_, 
...
vars == << mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
           ...
           old_orig_size, old_g1_start, r, s, lowseq_, local_wseq_, g_, seq, 
           signals, local_g1_start, lowseq, local_wseq_c, g1_, do_futex_wake_, 
           local_wseq_cv, g1_c, do_futex_wake, num_loops >>
...
check_state(self) == /\ pc[self] = "check_state"
                     ...
                     /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                     ...
                                     old_g1_start, r, s, lowseq_, local_wseq_, 
                                     g_, seq, signals, local_g1_start, lowseq, 
...
wait_for_wake(self) == /\ pc[self] = "wait_for_wake"
                       ...
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       ...
                                       lowseq_, local_wseq_, g_, seq, signals, 
...
dec_refs(self) == /\ pc[self] = "dec_refs"
                  ...
                  /\ UNCHANGED << mutex_index, g1_orig_size_index, wrefs_index, 
                                  ...
                                  r, s, lowseq_, local_wseq_, g_, seq, signals, 
...
quiesce_and_switch_check_for_no_reader(self) == /\ pc[self] = "quiesce_and_switch_check_for_no_reader"
                                                /\ IF ((local_wseq[self] - old_g1_start[self] - old_orig_size[self]) + g_size[Xor1(g1[self])]) = 0
                                                      THEN /\ did_switch_g1' = [did_switch_g1 EXCEPT ![self] = FALSE]
                                                           ...
                                                           /\ lowseq_' = [lowseq_ EXCEPT ![self] = Head(stack[self]).lowseq_]
...
quiesce_update_g1_start(self) == /\ pc[self] = "quiesce_update_g1_start"
                                 ...
                                 /\ pc' = [pc EXCEPT ![self] = "quiesce_lowseq"]
                                 /\ UNCHANGED << mutex_index, 
                                                 g1_orig_size_index, 
                                                 wrefs_index, g_signals_index, 
                                                 g_refs_index, futexes, wseq, 
...
quiesce_lowseq(self) == /\ pc[self] = "quiesce_lowseq"
                        /\ lowseq_' = [lowseq_ EXCEPT ![self] = ((old_g1_start[self] + old_orig_size[self]) * 2)]
                        /\ IF (futexes[g_refs_index[g1[self]]].state \div 2) > 0
                              THEN /\ pc' = [pc EXCEPT ![self] = "quiesce_wake_remaining"]
                              ELSE /\ pc' = [pc EXCEPT ![self] = "quiesce_flip_wseq"]
                        /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                        wrefs_index, g_signals_index, 
                                        g_refs_index, futexes, wseq, g1_start, 
                                        g_size, did_switch_g1, g1index, 
...
quiesce_wake_remaining(self) == /\ pc[self] = "quiesce_wake_remaining"
                                /\ futexes' = [futexes EXCEPT ![g_signals_index[g1[self]]].state = lowseq_[self]]
                                /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "futex_wake_all",
                                                                            pc        |->  "quiesce_flip_wseq",
                                                                            wake_all_index |->  wake_all_index[self] ] >>
                                                                        \o stack[self]]
                                   /\ wake_all_index' = [wake_all_index EXCEPT ![self] = g_signals_index[g1[self]]]
                                /\ pc' = [pc EXCEPT ![self] = "futex_wake_all_"]
                                /\ UNCHANGED << mutex_index, 
                                                g1_orig_size_index, 
                                                wrefs_index, g_signals_index, 
...
quiesce_flip_g1(self) == /\ pc[self] = "quiesce_flip_g1"
                         ...
                         /\ pc' = [pc EXCEPT ![self] = "quiesce_set_lowseq"]
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         wrefs_index, g_signals_index, 
                                         g_refs_index, futexes, wseq, g1_start, 
...
quiesce_set_lowseq(self) == /\ pc[self] = "quiesce_set_lowseq"
                            /\ futexes' = [futexes EXCEPT ![g_signals_index[g1[self]]].state = lowseq_[self]]
...
condvar_quiesce_and_switch_g1(self) == quiesce_and_switch_start(self)
                                          ...
                                          \/ quiesce_lowseq(self)
                                          \/ quiesce_wake_remaining(self)
                                          \/ quiesce_flip_wseq(self)
                                          \/ quiesce_flip_g1(self)
                                          \/ quiesce_set_lowseq(self)
...
cv_wait_fetch_seq(self) == /\ pc[self] = "cv_wait_fetch_seq"
                           ...
                           /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                           ...
                                           old_state, local_wseq, g1, 
                                           old_orig_size, old_g1_start, r, s, 
                                           lowseq_, signals, local_g1_start, 
                                           lowseq, local_wseq_c, g1_, 
                                           do_futex_wake_, local_wseq_cv, g1_c, 
                                           do_futex_wake, num_loops >>
...
cv_wait_inner_loop(self) == /\ pc[self] = "cv_wait_inner_loop"
                            /\ pc' = [pc EXCEPT ![self] = "cv_wait_reload_g1_start"]
                            /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                            wrefs_index, g_signals_index, 
                                            g_refs_index, futexes, wseq, 
                                            g1_start, g_size, did_switch_g1, 
...
cv_wait_reload_g1_start(self) == /\ pc[self] = "cv_wait_reload_g1_start"
                                 /\ local_g1_start' = [local_g1_start EXCEPT ![self] = g1_start]
                                 /\ lowseq' = [lowseq EXCEPT ![self] = IF (local_g1_start'[self] % 2) = g_[self] THEN signals[self] ELSE ClearBit(local_g1_start'[self], 0)]
...
cv_wait_spin_wait(self) == /\ pc[self] = "cv_wait_spin_wait"
                           /\ IF (signals[self] - lowseq[self]) < 2
                                 THEN /\ IF seq[self] < (local_g1_start[self] \div 2)
...
cv_wait_reload_signals(self) == /\ pc[self] = "cv_wait_reload_signals"
                                ...
                                /\ UNCHANGED << mutex_index, 
                                                ...
                                                old_orig_size, old_g1_start, r, 
                                                s, lowseq_, local_wseq_, g_, 
                                                seq, local_wseq_c, g1_, 
                                                do_futex_wake_, local_wseq_cv, 
                                                g1_c, do_futex_wake, num_loops >>

cv_wait_check_group_closed(self) == /\ pc[self] = "cv_wait_check_group_closed"
                                    /\ IF seq[self] < (g1_start \div 2)
                                          THEN /\ pc' = [pc EXCEPT ![self] = "cv_wait_done"]
                                          ELSE /\ IF (signals[self] - lowseq[self]) >= 2
...
cv_wait_prepare_to_block(self) == /\ pc[self] = "cv_wait_prepare_to_block"
                                  ...
                                  /\ pc' = [pc EXCEPT ![self] = "cv_wait_reload_signals_again"]
                                  /\ UNCHANGED << mutex_index, 
                                                  g1_orig_size_index, 
                                                  wrefs_index, g_signals_index, 
                                                  g_refs_index, wseq, g1_start, 
...
cv_wait_reload_signals_again(self) == /\ pc[self] = "cv_wait_reload_signals_again"
                                      /\ signals' = [signals EXCEPT ![self] = futexes[g_signals_index[g_[self]]].state]
                                      /\ local_g1_start' = [local_g1_start EXCEPT ![self] = g1_start]
                                      /\ lowseq' = [lowseq EXCEPT ![self] = IF (g1_start % 2) = g_[self] THEN signals'[self] ELSE ClearBit(local_g1_start'[self], 0)]
...
cv_wait_check_for_closed_group(self) == /\ pc[self] = "cv_wait_check_for_closed_group"
                                        /\ IF seq[self] < (local_g1_start[self] \div 2)
                                              ...
                                              ELSE /\ IF (signals[self] - lowseq[self]) >= 2
                                                         ...
                                                         ELSE /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "futex_wait",
                                                                                                          ...
                                                                                                          val       |->  val[self] ] >>
                                                                                                      \o stack[self]]
                                                                 /\ val' = [val EXCEPT ![self] = signals[self]]
...
cv_wait_inner_loop_done(self) == /\ pc[self] = "cv_wait_inner_loop_done"
                                 /\ IF seq[self] < (g1_start \div 2)
                                       THEN /\ pc' = [pc EXCEPT ![self] = "cv_wait_done"]
                                       ELSE /\ pc' = [pc EXCEPT ![self] = "cv_wait_inner_loop_done_cmp_exchg"]
                                 /\ UNCHANGED << mutex_index, 
                                                 g1_orig_size_index, 
                                                 wrefs_index, g_signals_index, 
                                                 g_refs_index, futexes, wseq, 
...
cv_wait_inner_loop_done_cmp_exchg(self) == /\ pc[self] = "cv_wait_inner_loop_done_cmp_exchg"
...
cv_wait_retake_mutex(self) == /\ pc[self] = "cv_wait_retake_mutex"
                              /\ /\ g_' = [g_ EXCEPT ![self] = Head(stack[self]).g_]
                                 ...
                                 /\ lowseq' = [lowseq EXCEPT ![self] = Head(stack[self]).lowseq]
...
cv_wait(self) == cv_wait_fetch_seq(self) \/ cv_wait_incr_wrefs(self)
                    ...
                    \/ cv_wait_inner_loop_done_cmp_exchg(self)
                    \/ cv_wait_done(self) \/ cv_wait_retake_mutex(self)
...
yield_loop(self) == /\ pc[self] = "yield_loop"
                    ...
                    /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                    ...
                                    orig_size, old_state, local_wseq, g1, 
                                    old_orig_size, old_g1_start, r, s, lowseq_, 
                                    local_wseq_c, g1_, do_futex_wake_, 
                                    local_wseq_cv, g1_c, do_futex_wake, 
                                    num_loops >>
...
\* Last modified Sun Jan 22 20:51:08 EST 2023 by malte
