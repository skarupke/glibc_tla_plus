...
quiesce_and_switch_start2:
  old_g1_start := g1_start;
  new_g1_start := old_g1_start + old_orig_size;
...
quiesce_update_g1_start:
  g1_start := g1_start + old_orig_size;
quiesce_flip_wseq:
  wseq := Xor1(wseq);
...
quiesce_set_lowseq:
  futexes[g_signals_index[g1]].state := new_g1_start;
quiesce_update_orig_size:
  old_orig_size := local_wseq - new_g1_start;
...
cv_wait_loop:
  ...
  cv_wait_check_group_closed:
    ...
    else if ((signals - local_g1_start) > 0)
    {
    cv_wait_cmp_exchg:
      if (futexes[g_signals_index[g]].state = signals)
      {
        futexes[g_signals_index[g]].state := signals - 1;
        goto cv_wait_done;
      }
      else
      {
...
cv_signal_add_signal:
  futexes[g_signals_index[g1]].state := futexes[g_signals_index[g1]].state + 1;
...
cv_broacast_check_size1:
  ...
  cv_broadcast_signal_g1:
    futexes[g_signals_index[g1]].state := futexes[g_signals_index[g1]].state + g_size[g1];
...
\* BEGIN TRANSLATION (chksum(pcal) = "55966bbc" /\ chksum(tla) = "948adcf5")
...
\* Label check_for_waiters of procedure cv_signal at line 355 col 3 changed to check_for_waiters_
...
\* Procedure variable local_wseq of procedure cv_signal at line 350 col 11 changed to local_wseq_c
\* Procedure variable g1 of procedure cv_signal at line 351 col 11 changed to g1_
\* Procedure variable do_futex_wake of procedure cv_signal at line 352 col 11 changed to do_futex_wake_
\* Procedure variable local_wseq of procedure cv_broadcast at line 404 col 11 changed to local_wseq_cv
\* Procedure variable g1 of procedure cv_broadcast at line 405 col 11 changed to g1_c
VARIABLES mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
          ...
          new_g1_start, local_wseq_, g, seq, signals, local_g1_start, 
...
vars == << mutex_index, g1_orig_size_index, wrefs_index, g_signals_index, 
           ...
           r, s, new_g1_start, local_wseq_, g, seq, signals, local_g1_start, 
           local_wseq_c, g1_, do_futex_wake_, local_wseq_cv, g1_c, 
           do_futex_wake, num_loops >>
...
Init == (* Global variables *)
        ...
        /\ r = [ self \in ProcSet |-> 0]
        /\ s = [ self \in ProcSet |-> 0]
        /\ new_g1_start = [ self \in ProcSet |-> 0]
        (* Procedure cv_wait *)
...
check_state(self) == /\ pc[self] = "check_state"
                     ...
                     /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                     ...
                                     new_g1_start, local_wseq_, g, seq, 
...
quiesce_and_switch_start2(self) == /\ pc[self] = "quiesce_and_switch_start2"
                                   /\ old_g1_start' = [old_g1_start EXCEPT ![self] = g1_start]
                                   /\ new_g1_start' = [new_g1_start EXCEPT ![self] = old_g1_start'[self] + old_orig_size[self]]
...
quiesce_and_switch_check_for_no_reader(self) == /\ pc[self] = "quiesce_and_switch_check_for_no_reader"
                                                /\ IF ((local_wseq[self] - old_g1_start[self] - old_orig_size[self]) + g_size[Xor1(g1[self])]) = 0
                                                      THEN /\ did_switch_g1' = [did_switch_g1 EXCEPT ![self] = FALSE]
                                                           ...
                                                           /\ new_g1_start' = [new_g1_start EXCEPT ![self] = Head(stack[self]).new_g1_start]
...
quiesce_update_g1_start(self) == /\ pc[self] = "quiesce_update_g1_start"
                                 /\ g1_start' = g1_start + old_orig_size[self]
...
quiesce_flip_g1(self) == /\ pc[self] = "quiesce_flip_g1"
                         ...
                         /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                         ...
                                         val, wake_index, wake_all_index, 
                                         old_state_, old_state_u, orig_size, 
                                         old_state, local_wseq, old_orig_size, 
                                         old_g1_start, r, s, new_g1_start, 
                                         local_wseq_, g, seq, signals, 
                                         local_g1_start, local_wseq_c, g1_, 
                                         do_futex_wake_, local_wseq_cv, g1_c, 
...
quiesce_set_lowseq(self) == /\ pc[self] = "quiesce_set_lowseq"
                            /\ futexes' = [futexes EXCEPT ![g_signals_index[g1[self]]].state = new_g1_start[self]]
...
quiesce_update_orig_size(self) == /\ pc[self] = "quiesce_update_orig_size"
                                  /\ old_orig_size' = [old_orig_size EXCEPT ![self] = local_wseq[self] - new_g1_start[self]]
...
quiesce_update_size(self) == /\ pc[self] = "quiesce_update_size"
                             ...
                             /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                             wrefs_index, g_signals_index, 
                                             futexes, wseq, g1_start, g1index, 
                                             waiters, busy, spinlock_locked, 
                                             cv_spinlock_locked, wait_index, 
                                             val, wake_index, wake_all_index, 
                                             old_state_, old_state_u, 
                                             orig_size, old_state, local_wseq_, 
                                             g, seq, signals, local_g1_start, 
                                             local_wseq_c, g1_, do_futex_wake_, 
                                             local_wseq_cv, g1_c, 
                                             do_futex_wake, num_loops >>
...
cv_wait_fetch_seq(self) == /\ pc[self] = "cv_wait_fetch_seq"
                           ...
                           /\ wseq' = wseq + 2
                           /\ g' = [g EXCEPT ![self] = local_wseq_'[self] % 2]
...
cv_wait_loop(self) == /\ pc[self] = "cv_wait_loop"
                      /\ signals' = [signals EXCEPT ![self] = futexes[g_signals_index[g[self]]].state]
                      ...
                      /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                      ...
                                      old_g1_start, r, s, new_g1_start, 
                                      local_wseq_, g, seq, local_g1_start, 
                                      local_wseq_c, g1_, do_futex_wake_, 
                                      local_wseq_cv, g1_c, do_futex_wake, 
                                      num_loops >>
...
cv_wait_check_group_closed(self) == /\ pc[self] = "cv_wait_check_group_closed"
                                    /\ IF seq[self] < g1_start
                                          THEN /\ pc' = [pc EXCEPT ![self] = "cv_wait_done"]
                                          ELSE /\ IF (signals[self] - local_g1_start[self]) > 0
                                                     THEN /\ pc' = [pc EXCEPT ![self] = "cv_wait_cmp_exchg"]
                                                     ELSE /\ pc' = [pc EXCEPT ![self] = "cv_wait_block"]
                                    /\ UNCHANGED << mutex_index, 
                                                    g1_orig_size_index, 
                                                    wrefs_index, 
                                                    g_signals_index, futexes, 
...
cv_wait_cmp_exchg(self) == /\ pc[self] = "cv_wait_cmp_exchg"
                           /\ IF futexes[g_signals_index[g[self]]].state = signals[self]
                                 THEN /\ futexes' = [futexes EXCEPT ![g_signals_index[g[self]]].state = signals[self] - 1]
                                      /\ pc' = [pc EXCEPT ![self] = "cv_wait_done"]
                                      /\ UNCHANGED signals
                                 ELSE /\ signals' = [signals EXCEPT ![self] = futexes[g_signals_index[g[self]]].state]
...
cv_wait_block(self) == /\ pc[self] = "cv_wait_block"
                       /\ /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "futex_wait",
                          ...
                          /\ wait_index' = [wait_index EXCEPT ![self] = g_signals_index[g[self]]]
                       /\ pc' = [pc EXCEPT ![self] = "check_state"]
                       /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                       wrefs_index, g_signals_index, futexes, 
                                       wseq, g1_start, g_size, did_switch_g1, 
...
cv_wait_retake_mutex(self) == /\ pc[self] = "cv_wait_retake_mutex"
                              ...
                              /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                              ...
                                              old_state, local_wseq, g1, 
                                              old_orig_size, old_g1_start, r, 
                                              s, new_g1_start, local_wseq_c, 
                                              g1_, do_futex_wake_, 
                                              local_wseq_cv, g1_c, 
                                              do_futex_wake, num_loops >>

cv_wait(self) == cv_wait_fetch_seq(self) \/ cv_wait_incr_wrefs(self)
                    \/ cv_wait_loop(self) \/ cv_wait_reload_g1_start(self)
                    \/ cv_wait_check_group_closed(self)
                    \/ cv_wait_cmp_exchg(self) \/ cv_wait_block(self)
                              ...
                              /\ IF g_size[g1_[self]] = 0
                                    THEN /\ /\ g1' = [g1 EXCEPT ![self] = g1_[self]]
                                            ...
                                            /\ stack' = [stack EXCEPT ![self] = << [ procedure |->  "condvar_quiesce_and_switch_g1",
                                                                                     ...
                                                                                     r         |->  r[self],
                                                                                     s         |->  s[self],
                                                                                     new_g1_start |->  new_g1_start[self],
                                         ...
                                         /\ r' = [r EXCEPT ![self] = 0]
                                         /\ s' = [s EXCEPT ![self] = 0]
                                         /\ new_g1_start' = [new_g1_start EXCEPT ![self] = 0]
...
cv_signal_add_signal(self) == /\ pc[self] = "cv_signal_add_signal"
                              /\ futexes' = [futexes EXCEPT ![g_signals_index[g1_[self]]].state = futexes[g_signals_index[g1_[self]]].state + 1]
...
cv_broadcast_signal_g1(self) == /\ pc[self] = "cv_broadcast_signal_g1"
                                /\ futexes' = [futexes EXCEPT ![g_signals_index[g1_c[self]]].state = futexes[g_signals_index[g1_c[self]]].state + g_size[g1_c[self]]]
...
yield_start(self) == /\ pc[self] = "yield_start"
                     ...
                     /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                     ...
                                     g1index, waiters, busy, spinlock_locked, 
                                     cv_spinlock_locked, wait_index, val, 
                                     wake_index, wake_all_index, old_state_u, 
                                     orig_size, old_state, local_wseq, g1, 
                                     old_orig_size, old_g1_start, r, s, 
                                     new_g1_start, local_wseq_, g, seq, 
                                     signals, local_g1_start, local_wseq_c, 
...
yield_done(self) == /\ pc[self] = "yield_done"
                    ...
                    /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                    ...
                                    orig_size, old_state, local_wseq, g1, 
                                    old_orig_size, old_g1_start, r, s, 
                                    new_g1_start, local_wseq_, g, seq, signals, 
                                    local_g1_start, local_wseq_c, g1_, 
                                    do_futex_wake_, local_wseq_cv, g1_c, 
                                    do_futex_wake, num_loops >>
...
proc_done(self) == /\ pc[self] = "proc_done"
                   ...
                   /\ UNCHANGED << mutex_index, g1_orig_size_index, 
                                   wrefs_index, g_signals_index, futexes, wseq, 
                                   g1_start, g_size, did_switch_g1, g1index, 
                                   waiters, busy, spinlock_locked, 
                                   ...
                                   r, s, new_g1_start, local_wseq_, g, seq, 
...
\* Last modified Sun Jan 22 21:00:32 EST 2023 by malte
